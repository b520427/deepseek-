...
export function useAccountActions({ apiFetch, t, onMessage, onRefresh, config, fetchAccounts, resolveAccountIdentifier }) {
    ...
    const batchImportAccounts = async (text) => {
        if (!text.trim()) return
        setLoading(true)
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const password = line.substring(idx + 4).trim()
            if (!email || !password) { errors++; continue }
        ...
        const msg = (t('accountManager.batchImportSuccess') || '导入完成：新增 {added} 个，跳过 {skipped} 个，失败 {errors} 个')
            .replace('{added}', added).replace('{skipped}', skipped).replace('{errors}', errors)
